...
function extractDaxReferences(expression, currentTable, allNodes) {
  ...
  // Requires either a quoted 'Table Name' or a valid identifier before [
  const qualifiedPattern = /(?:'([^']+)'|([A-Za-z_]\w*))\[([^\]]+)\]/g;
  let match;
  ...
  // Match unqualified [FieldName] — standalone [Name] not preceded by a word char or quote

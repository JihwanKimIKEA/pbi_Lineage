...
export function detectFieldParameter(table) {
  ...
  if (nameofFields.length === 0) {
    // Fallback: detect field parameter via ParameterMetadata on columns
    const hasParameterMetadata = columns.some(col => col.hasParameterMetadata);

...
function parseColumnBlock(lines, startIndex, name) {
  const col = { name, dataType: '', sourceColumn: '', expression: null, isHidden: false, hasParameterMetadata: false };
  ...
  while (i < lines.length) {
    ...
    // Detect extendedProperty ParameterMetadata
    if (/^extendedProperty\s+ParameterMetadata\b/i.test(trimmed)) {
      col.hasParameterMetadata = true;
      i++;
      continue;
    }
...
export function extractMDataSource(mExpression) {
  ...
  if (bqMatch && !result.type) {
    ...
    if (bqMatch[1]) {
      // Direct string argument: GoogleBigQuery.Database("project-id")
      result.server = bqMatch[1];
    } else if (bqMatch[2]) {
      // Options record: GoogleBigQuery.Database([BillingProject="value", ...])
      const billingMatch = bqMatch[2].match(/BillingProject\s*=\s*"([^"]+)"/);
      if (billingMatch) {
        result.server = billingMatch[1];
      }
    } else if (bqMatch[3]) {
      result.server = bqMatch[3];
    }
  }

...
describe('parseSemanticModelReference', () => {
  ...
  it('parses byPath reference without trailing slash', () => {
    const content = JSON.stringify({
      datasetReference: { byPath: { path: '../prod_dataset_picking_dashboard_v1.SemanticModel' } }
    });
    expect(parseSemanticModelReference(content)).toBe('../prod_dataset_picking_dashboard_v1.SemanticModel');
  });

  it('parses byPath with schema and version fields', () => {
    const content = JSON.stringify({
    ...
    expect(parseSemanticModelReference(content)).toBe('../MyModel.SemanticModel');
  });

  it('returns null for byConnection reference (no byPath)', () => {
    const content = JSON.stringify({
      datasetReference: { byConnection: { connectionString: 'some-guid' } }
    });
    expect(parseSemanticModelReference(content)).toBeNull();
  });

...
import {
  ...
  identifyProjectStructure, findDefinitionPbir, parseSemanticModelReference,
} from '@pbip-lineage/core';

// Browser-specific file reader
import { openProjectFolder, loadSelectedReport, loadSemanticModelFolder, hasFileSystemAccess, walkDirectory } from './parser/pbipReader.js';
...
function updateValueCounter() {
  ...
  if (parts.length > 0) {
    ...
    counter.innerHTML = `${parts.join(' &middot; ')} in this session`;
...
function handleBulkExport() {
  ...
  const footer = '\n"Traced with PBIP Lineage Explorer"';
...
async function handleOpenFolder() {
  try {
    ...
    // Auto-resolve: if report loaded without model and we know the model path, prompt for it
    if (!projectResult.modelStructure && projectResult.semanticModelPath && hasFileSystemAccess()) {
      const modelHint = projectResult.semanticModelPath.replace(/^\.\.\//, '').replace(/\/$/, '');
      showLoading(`Now select the ${modelHint} folder...`);

      try {
        const pickerOptions = { mode: 'read' };
        // Use startIn to hint the user toward the report's parent directory
        if (projectResult.reportHandle) {
          pickerOptions.startIn = projectResult.reportHandle;
        }
        const modelDirHandle = await window.showDirectoryPicker(pickerOptions);
        const modelFiles = await walkDirectory(modelDirHandle, '');
        const modelStructure = identifyProjectStructure(modelFiles);
        projectResult.modelStructure = modelStructure;
        projectResult.modelName = modelDirHandle.name;
      } catch (err) {
        // User cancelled or error — continue without model (banner will show)
        if (err.name !== 'AbortError') {
          console.warn('Semantic model auto-load skipped:', err);
        }
      }
    }
...
async function handleDroppedDirectory(dirHandle) {
  try {
    ...
    // If the dropped folder is itself a .Report folder, handle directly
    const lowerName = dirHandle.name.toLowerCase();
    if (lowerName.endsWith('.report')) {
      const reportFiles = await walkDirectory(dirHandle, '');
      const reportStructure = identifyProjectStructure(reportFiles);
